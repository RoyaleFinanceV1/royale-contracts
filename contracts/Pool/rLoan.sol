...
contract rLoan is ReentrancyGuard{
    ...
    function approveLoan(uint _loanID) external onlyWallet() {
       uint royaleBalance=royale.calculateTotalToken(false);
        uint approvedTotal;
        uint requestedTotal;
        uint decimal;
        for(uint8 i=0;i<N_COINS;i++) {
            decimal=tokens[i].decimals();
            approvedTotal=approvedTotal.add(totalApprovedLoan[i].mul(10**18).div(10**decimal));
            requestedTotal=requestedTotal.add((transactions[_loanID].tokenAmounts[i]).mul(10**18).div(10**decimal));
        }
        require(approvedTotal.add(requestedTotal) < royaleBalance,"Can not approve that much amount");
    ...
    function withdrawLoan( uint256[N_COINS] calldata amounts,uint _loanID) external nonReentrant{
        ...
        for(uint8 i=0;i<3;i++){
            decimal=tokens[i].decimals();
            total=total.add(amounts[i].mul(10**18).div(10**decimal));
        }
        require(total<royale.calculateTotalToken(false),"Not enough balance");
        ...
        for(uint8 i=0; i<N_COINS; i++) {
            if(amounts[i] > 0) {
                ...
                totalApprovedLoan[i]=totalApprovedLoan[i].sub(amounts[i]);
    ...
    function repayLoan(uint256[N_COINS] calldata _amounts, uint _loanId) external nonReentrant{
        ...
        bool b = royale._loanRepayment(_amounts);
        require(b,"Loan Payment not succesfull");
        uint counter=0;
        for(uint i=0;i<N_COINS;i++) {
            if(_amounts[i]!=0) {
                tokens[i].transferFrom(transactions[_loanId].iGamingCompany, address(royale), _amounts[i]);
